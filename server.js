...
const fetch = require('node-fetch'); // v2.x required for CommonJS
...
// Use your NEW Resend API key
const RESEND_API_KEY = "re_QjyGwbiY_QBEnySF8TCVviBBAHG8nixkm";

// The inbox that should RECEIVE all CU submissions
...
// Health check
app.get('/', (req, res) => {
  res.send('CU Email API is running (Resend)');
});
...
Submitted at: ${new Date().toLocaleString()}
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [CU_INBOX],     // now sends to churchunlimited2020@gmail.com
        subject,
        text
      })
    });
...
// ---------------------------------------------------------
// 2) SPIRITUAL GIFTS ASSESSMENT ENDPOINT
// ---------------------------------------------------------
app.post('/api/gifts-assessment', async (req, res) => {
  try {
    ...
    const allGiftsText =
      allGifts.map(g => `- ${g.name}: ${g.score}`).join('\n');

    const topGiftsText =
      topGifts.map((g, i) => `${i + 1}. ${g.name} (${g.score})`).join('\n');
